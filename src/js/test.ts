...
const OPCODES = [
	...
	//#0x84:
	function (parentObj) {
		var dirtySum = parentObj.registerA + (parentObj.registerH);
		parentObj.FHalfCarry = ((dirtySum & 0xF) < (parentObj.registerA & 0xF));
		parentObj.FCarry = (dirtySum > 0xFF);
		parentObj.registerA = dirtySum & 0xFF;
		parentObj.FZero = (parentObj.registerA == 0);
	...
	//#0x85:
	function (parentObj) {
		var dirtySum = parentObj.registerA + (parentObj.registerL);
		parentObj.FHalfCarry = ((dirtySum & 0xF) < (parentObj.registerA & 0xF));
		parentObj.FCarry = (dirtySum > 0xFF);
		parentObj.registerA = dirtySum & 0xFF;
		parentObj.FZero = (parentObj.registerA == 0);
	...
	//#0x8C:
	function (parentObj) {
		var tempValue = (parentObj.registerH);
	...
	//#0x8D:
	function (parentObj) {
		var tempValue = (parentObj.registerL);
		var dirtySum = parentObj.registerA + tempValue + ((parentObj.FCarry) ? 1 : 0);
		parentObj.FHalfCarry = ((parentObj.registerA & 0xF) + (tempValue & 0xF) + ((parentObj.FCarry) ? 1 : 0) > 0xF);
		parentObj.FCarry = (dirtySum > 0xFF);
		parentObj.registerA = dirtySum & 0xFF;
	...
	//#0x9C:
	function (parentObj) {
		var temp_var = parentObj.registerH;
	...
	//#0x9D:
	function (parentObj) {
		var dirtySum = parentObj.registerA - (parentObj.registerL) - ((parentObj.FCarry) ? 1 : 0);
		parentObj.FHalfCarry = ((parentObj.registerA & 0xF) - (parentObj.registerL & 0xF) - ((parentObj.FCarry) ? 1 : 0) < 0);
		parentObj.FCarry = (dirtySum < 0);
		parentObj.registerA = dirtySum & 0xFF;
		parentObj.FZero = (parentObj.registerA == 0);
		parentObj.FSubtract = true;
...
const setState = (state) => {
    gameboy.CPU.registers.a.value.value = state.registerA;
    gameboy.CPU.registers.b.value.value = state.registerB;
    gameboy.CPU.registers.c.value.value = state.registerC;
    gameboy.CPU.registers.d.value.value = state.registerD;
    ...
    gameboy.CPU.flags.subtraction = state.FSubtract ? 1 : 0;
};

const assertEqual = (instruction, state, original) => {
    try {
        if (gameboy.CPU.registers.a.value.value !== state.registerA) throw new Error('a');
        if (gameboy.CPU.registers.b.value.value !== state.registerB) throw new Error('b');
        ...
        if (gameboy.CPU.flags.subtraction !== (state.FSubtract ? 1 : 0)) throw new Error('fs');
    } catch(e){
        console.error(`Mismatched instruction (${instruction.toString(16).padStart(2,'0')}) (${e}): ${logStates(state, original)}`);
    }
};

const logStates = (state, original) => {
    return `
        a: ${original.registerA}, ${state.registerA}, ${gameboy.CPU.registers.a.value.value},
        b: ${original.registerB}, ${state.registerB}, ${gameboy.CPU.registers.b.value.value},
        ...
        zero: ${original.FZero ? 1 : 0}, ${state.FZero ? 1 : 0}, ${gameboy.CPU.flags.zero},
        carry: ${original.FCarry ? 1 : 0}, ${state.FCarry ? 1 : 0}, ${gameboy.CPU.flags.carry},
        halfcarry: ${original.FHalfCarry ? 1 : 0}, ${state.FHalfCarry ? 1 : 0}, ${gameboy.CPU.flags.halfCarry},
        sub: ${original.FSubtract ? 1 : 0}, ${state.FSubtract ? 1 : 0}, ${gameboy.CPU.flags.subtraction},
    `;
};


// ADD/ADC: 0x80 -> 0x8F, except 0x86, 0x8E
// SUB/SBC: 0x90 -> 0x9F, except 0x96, 0x9E
//
for (let i=0x80;i<=0x9F;i++){
    if ((i&0xF) === 0x06 || (i&0xF) === 0x0E) continue;
    for (const state of states){
        ...
        for (let j=0x00;j<=0xFF;j++){
            ...
            theirs.registerA = j;
            setState(theirs);
            gameboy.CPU.ops[i.toString(16).padStart(2, '0').toUpperCase()]();
            OPCODES[i](theirs);
            assertEqual(i, theirs, state);
        }
    }
}
...
for (let i=0x00;i<=0xFF;i++){
    ...
    for (const state of states){
        const theirs = {...state};
        setState(theirs);
        gameboy.CPU.CBOps[i.toString(16).padStart(2, '0').toUpperCase()]();
        CBOPCODE[i](theirs);
        assertEqual(i, theirs, state);
    }
}